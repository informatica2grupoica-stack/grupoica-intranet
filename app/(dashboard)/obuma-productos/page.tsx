...
const RenderForm = ({ 
  ...
  <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-slate-200 space-y-6 max-w-5xl mx-auto my-4">
    <div className="p-4 bg-[#059669] rounded-2xl text-white shadow-md flex justify-between items-center">
      <div>
    ...
    <div className="grid grid-cols-4 gap-4">
      {[{ k: 'c1', l: '1. TIPO' }, { k: 'c2', l: '2. ATRIBUTO' }, { k: 'c3', l: '3. MEDIDA' }, { k: 'c4', l: '4. MARCA' }].map((item) => (
        <div key={item.k} className="flex flex-col gap-1">
          ...
          <input 
            className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold uppercase outline-none focus:border-[#059669] transition-colors"
    ...
    <div className="grid grid-cols-3 gap-6">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[9px] font-black text-[#059669] uppercase italic">SKU / Código Comercial</label>
    ...
    <div className="grid grid-cols-3 gap-6 items-end">
      ...
      <div className="flex-1">
        ...
        <input type="number" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none border-b-2 border-b-[#059669]" value={editForm.precio_venta || 0} onChange={(e) => setEditForm({...editForm, precio_venta: e.target.value})} />
      </div>
    </div>
    ...
    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
      ...
      <button 
        ...
        className="flex items-center gap-3 bg-[#059669] text-white px-12 py-4 rounded-2xl text-xs font-black uppercase shadow-xl hover:bg-blue-800 transition-all active:scale-95 disabled:opacity-50"
      >
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 bg-[#F3F4F6] min-h-screen">
      ...
      <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-[300px]">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:border-[#059669] transition-all"
              value={search}
          ...
          <Link 
            ...
            className="bg-[#059669] hover:bg-[#047857] text-white p-4 rounded-2xl shadow-lg transition-all active:scale-95 flex items-center justify-center"
          >
            <Plus size={24} />
          </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-2 rounded-xl bg-slate-50 text-slate-400 hover:bg-[#059669] hover:text-white disabled:opacity-30 transition-all"><ChevronLeft size={20} /></button>
            ...
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(prev => prev + 1)} className="p-2 rounded-xl bg-slate-50 text-slate-400 hover:bg-[#059669] hover:text-white disabled:opacity-30 transition-all"><ChevronRight size={20} /></button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[800px]">
          ...
          <tbody className="divide-y divide-slate-50">
            {loading ? (
              <tr><td colSpan={7} className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-[#059669]" /></td></tr>
            ) : currentItems.length === 0 ? (
              <tr><td colSpan={7} className="py-20 text-center text-slate-400">No se encontraron productos</td></tr>
            ) : (
              currentItems.map((prod) => (
                <React.Fragment key={prod.id}>
                  <tr className={`hover:bg-slate-50/50 transition-all ${editingId === prod.id ? 'bg-[#ECFDF5]/20' : ''}`}>
                    <td className="px-8 py-5">
                      <div className="text-sm font-black text-slate-700 uppercase italic leading-tight">{prod.nombre}</div>
                      <div className="flex flex-wrap gap-1 mt-1">
                        <span className="text-[8px] font-black text-[#059669] uppercase bg-[#ECFDF5] px-1.5 py-0.5 rounded">{prod.categoria_nombre || 'Sin Categoría'}</span>
                    ...
                    <td className="px-4 py-5 text-right font-black text-[#059669] text-sm italic">
                      {formatPrice(prod.precio_total || 0)}
                    </td>
                    <td className="px-4 py-5 text-center">
                      <span className={`text-[9px] font-black px-2 py-1 rounded-full ${prod.tipo === 'Producto' ? 'bg-[#D1FAE5] text-[#059669]' : 'bg-purple-100 text-purple-600'}`}>
                        {prod.tipo || 'Producto'}
                      </span>
                    </td>
                    <td className="px-8 py-5 text-center">
                      <button onClick={() => handleEditClick(prod)} className="p-2.5 bg-slate-100 rounded-xl text-slate-400 hover:bg-[#059669] hover:text-white transition-all active:scale-90 shadow-sm">
