...
export default function ObumaProductosListado() {
  ...
  // --- LÓGICA DE NORMALIZACIÓN (LAS 4 PIEZAS) ---
  useEffect(() => {
  ...
  // Componente del Formulario Unificado (Nuevo y Edición)
  const RenderForm = ({ id }: { id: string }) => (
    <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-slate-200 space-y-6 max-w-5xl mx-auto my-4">
      <div className="p-4 bg-[#00338d] rounded-2xl text-white shadow-md flex justify-between items-center">
        ...
        {id === "nuevo" ? (
          <div className="bg-amber-400 text-[#00338d] px-4 py-1 rounded-full text-[10px] font-black italic shadow-sm">NUEVO REGISTRO</div>
        ) : (
          <div className="bg-blue-400/20 px-4 py-1 rounded-full text-[10px] font-black italic">MODO EDICIÓN</div>
        )}
      </div>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
        ...
        >
          ...
          {id === "nuevo" ? "Crear en Obuma" : "Guardar Cambios"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 bg-[#f8fafc] min-h-screen">
      ...
      {/* --- FORMULARIO PARA NUEVO (SI ESTÁ ACTIVO) --- */}
          ...
          <tbody className="divide-y divide-slate-50">
            {loading ? (
              ...
              <React.Fragment key={prod.producto_id}>
                ...
                {(editingId === prod.producto_id && editingId !== "nuevo") && (
