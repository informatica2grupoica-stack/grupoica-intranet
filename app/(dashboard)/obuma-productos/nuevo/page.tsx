...
import { Save, Loader2, CheckCircle2, AlertCircle, Wrench } from "lucide-react";
...
export default function NuevoProductoForm() {
  ...
  // 1. CAMPOS PARA CONSTRUCCIÓN AUTOMÁTICA SEGÚN PROCEDIMIENTO
  const [piezas, setPiezas] = useState({
    tipo: "",
    caracteristica: "",
    valorMedida: "",
    unidadMedida: "MT", // MT, CM, MM, "
    marca: ""
  });

  const [form, setForm] = useState({
    nombre: "", // Se autogenera
    tipo: "Producto",
    sku: "",
    categoria_id: "",
    subcategoria_id: "",
  ...
  // 2. LÓGICA DE GENERACIÓN AUTOMÁTICA DE NOMBRE (ESTÁNDAR SKU)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus({ 
          type: 'success', 
          msg: `PRODUCTO CREADO EXITOSAMENTE: ${form.nombre}` 
        });

        // Limpiar formulario y piezas
    ...
    <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-slate-100 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="bg-slate-100 px-4 py-2 rounded-xl border border-slate-200">
           <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Procedimiento SKU Estándar</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* BLOQUE DE CONSTRUCCIÓN AUTOMÁTICA */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100">
          <div className="flex flex-col gap-2">
            ...
            <input 
              required
              className="p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold uppercase outline-none focus:border-[#00338d]"
              placeholder="EJ: ZINC"
              value={piezas.tipo}
              onChange={(e) => setPiezas({...piezas, tipo: e.target.value})}
            />
          </div>
          <div className="flex flex-col gap-2">
            ...
            <input 
              required
              className="p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold uppercase outline-none focus:border-[#00338d]"
              placeholder="EJ: ACANALADO"
              value={piezas.caracteristica}
              onChange={(e) => setPiezas({...piezas, caracteristica: e.target.value})}
            />
          </div>
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-1">
              <input 
                className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-[#00338d]"
                placeholder="3,66"
                value={piezas.valorMedida}
                onChange={(e) => setPiezas({...piezas, valorMedida: e.target.value})}
              />
              <select 
                className="p-3 bg-white border border-slate-200 rounded-xl font-black text-[10px]"
                value={piezas.unidadMedida}
                onChange={(e) => setPiezas({...piezas, unidadMedida: e.target.value})}
              >
          ...
          <div className="flex flex-col gap-2">
            ...
            <input 
              className="p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold uppercase outline-none focus:border-[#00338d]"
              placeholder="EJ: POLPAICO"
              value={piezas.marca}
              onChange={(e) => setPiezas({...piezas, marca: e.target.value})}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-black uppercase text-slate-400 italic">Código SKU *</label>
            <input 
              required
              className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-mono uppercase focus:border-[#00338d] outline-none"
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <input 
                type="number"
                required
                className="w-full p-4 pl-8 bg-white border border-slate-200 rounded-2xl text-lg font-black text-center outline-none focus:border-[#00338d]"
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <input 
                type="number"
                required
                className="w-full p-4 pl-8 bg-white border-2 border-[#00338d]/20 rounded-2xl text-lg font-black text-[#00338d] text-center outline-none focus:border-[#00338d]"
