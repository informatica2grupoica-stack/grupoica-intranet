...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const Brand = ({ dark = true }: { dark?: boolean }) => (
    <div className="flex items-center gap-3">
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#1D4ED8] to-[#06B6D4] flex items-center justify-center shadow-lg shadow-blue-900/30">
        <Sparkles className="w-5 h-5 text-white" />
      </div>
      <div className="leading-tight">
        <p className={`font-black text-[15px] tracking-tight ${dark ? "text-white" : "text-slate-800"}`}>
          Comercial <span className="text-[#06B6D4]">MP</span>
        </p>
  ...
  const UserCard = () => (
    <div className="relative">
        ...
        <div className="absolute bottom-full mb-2 left-0 right-0 bg-white rounded-2xl shadow-2xl border border-slate-200 p-3 z-50">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-[#1D4ED8]" /> Privilegios
            </span>
          ...
          <div className="mb-2">
            ...
            <p className="text-xs font-bold text-[#1D4ED8]">{userRol ? (ROL_LABEL[userRol] || userRol) : "—"}</p>
          </div>
          <div className="space-y-1">
            {permActivos.length ? permActivos.map(([k]) => (
              <div key={k} className="flex items-center gap-2 text-[11px] text-slate-600">
                <span className="w-1.5 h-1.5 rounded-full bg-[#06B6D4]" /> {PERM_LABELS[k] || k}
              </div>
      ...
      <div className="flex items-center gap-3 p-2.5 rounded-2xl bg-white/5 hover:bg-white/10 transition-all border border-white/5">
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#1D4ED8] to-[#06B6D4] flex items-center justify-center text-white font-bold text-xs shadow-lg shadow-blue-900/40">
        ...
        <button onClick={() => setShowPerms(s => !s)} className="flex-1 overflow-hidden text-left">
          ...
          {userRol && (
            <span className="inline-flex items-center gap-1 mt-0.5 text-[8px] font-bold uppercase text-[#06B6D4]">
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      {/* SIDEBAR */}
      <aside className="w-64 fixed h-full z-20 flex flex-col bg-gradient-to-b from-[#0F172A] to-[#1E293B] border-r border-white/5">
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  ...
                  const cls = `group flex items-center justify-between px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all relative ${
                    isActive ? "bg-gradient-to-r from-[#1D4ED8]/25 to-transparent text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                  }`;
                  const content = (
                    <>
                      {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 rounded-r bg-[#06B6D4]" />}
                      <span className="flex items-center gap-3">
                        <item.icon className={`w-[18px] h-[18px] transition-colors ${isActive ? "text-[#06B6D4]" : "text-slate-500 group-hover:text-[#06B6D4]"}`} />
                        {item.name}
                      </span>
      ...
      <main className="flex-1 ml-64 p-6 md:p-8">
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-7">
            <div className="flex items-center gap-3">
              <span className="font-mono text-[10px] font-bold tracking-wider text-white bg-gradient-to-r from-[#1D4ED8] to-[#06B6D4] px-2.5 py-1 rounded-md shadow-sm">
                {meta.id}
              </span>
              <div>
