...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-160px)] bg-white border border-slate-200 rounded-[3rem] overflow-hidden shadow-2xl shadow-blue-900/5 relative">
      ...
      {/* BARRA LATERAL */}
      <aside className="w-80 bg-[#2563EB] flex flex-col relative overflow-hidden">
            ...
            return (
              ...
              >
                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black text-xs ${
                  seleccionado?.user_id === u.user_id ? 'bg-[#2563EB] text-white' : 'bg-white/10 text-white'
                }`}>
                ...
                {count > 0 && (
                  <div className="bg-[#2563EB] text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black shadow-lg animate-bounce">
                    {count}
                  </div>
                )}
              </button>
        ...
        {seleccionado ? (
          <>
            <div className="px-10 py-6 border-b border-slate-100 flex items-center gap-4 bg-white/80 backdrop-blur-md">
              <div className="relative">
                <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center font-black text-[#2563EB]">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#2563EB] font-black uppercase tracking-widest">{seleccionado.cargo}</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-6 bg-[#fcfdfe]">
              {cargandoMensajes ? (
                <div className="flex justify-center items-center h-full">
                  <Loader2 className="w-8 h-8 animate-spin text-[#2563EB] opacity-20" />
                </div>
              ) : (
                mensajes.map((m, idx) => (
                  <div key={idx} className={`flex ${m.emisor_id === myUserId ? 'justify-end' : 'justify-start'}`}>
                    <div className="max-w-[65%]">
                      <div className={`px-6 py-4 rounded-[2rem] text-sm font-medium shadow-sm animate-in slide-in-from-bottom-2 ${
                        ...
                        ? 'bg-[#2563EB] text-white rounded-tr-none shadow-blue-900/10' 
                      ...
                      <p className={`text-[9px] mt-2 font-bold uppercase text-slate-400 ${m.emisor_id === myUserId ? 'text-right mr-2' : 'text-left ml-2'}`}>
                        ...
                        {m.emisor_id === myUserId && <CheckCheck className={`w-3 h-3 inline ml-1 ${m.leido ? 'text-[#60A5FA]' : 'text-slate-300'}`} />}
                      </p>
                    </div>
                  </div>
                ))
              ...
              <div className="relative flex items-center bg-slate-100/80 rounded-[2rem] p-2 pr-3 border-2 border-slate-200/50 focus-within:border-[#2563EB]/20 focus-within:bg-white focus-within:ring-8 ring-[#EFF6FF]/50 transition-all duration-300">
                ...
                <button 
                  ...
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-lg ${
                    nuevoMensaje.trim() 
                    ? 'bg-[#2563EB] text-white shadow-blue-900/30 hover:scale-105 active:scale-95' 
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            <div className="w-24 h-24 bg-[#EFF6FF] rounded-[2.5rem] flex items-center justify-center mb-6">
              <MessageSquare className="w-10 h-10 text-[#2563EB] opacity-20" />
            </div>
