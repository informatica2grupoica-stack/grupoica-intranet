...
const tipoIcon = (t: string) => {
  ...
  if (t === "Notebook") return <Laptop size={16} className="text-[#2563EB]" />;
      ...
      <div className="bg-white rounded-[2rem] p-5 shadow-sm border border-slate-100">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={exportarExcel}
            className="px-3 py-2.5 text-xs font-bold text-white bg-[#2563EB] rounded-xl flex items-center gap-1.5 hover:bg-[#1D4ED8] transition-all shadow-sm">
            ...
            <div key={grupo.id} className="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-sm hover:border-[#A7F3D0] transition-all group/card">
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white text-base font-black group-hover/card:bg-[#2563EB] transition-colors">
                    {grupo.nombre.substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <h4 className="font-black text-slate-800 uppercase text-sm">{grupo.nombre}</h4>
                    <p className="text-[10px] text-[#2563EB] font-bold flex items-center gap-1">
              ...
              <div className="space-y-2.5">
                {grupo.equipos.map((eq: any) => (
                  <div key={eq.id}
                    className={`group/eq relative flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-transparent transition-all ${canEdit ? "hover:border-[#EFF6FF] hover:bg-white cursor-pointer" : "cursor-default"}`}
                    onClick={() => canEdit && openEdit(eq)}
                  >
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${eq.tipo === "Telefono" ? "bg-amber-100" : eq.tipo === "Monitor" ? "bg-purple-100" : "bg-[#EFF6FF]"}`}>
                      {tipoIcon(eq.tipo)}
                    </div>
                ...
                {dispositivosFiltrados.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${d.tipo === "Telefono" ? "bg-amber-100" : d.tipo === "Monitor" ? "bg-purple-100" : "bg-[#EFF6FF]"}`}>
                        {tipoIcon(d.tipo)}
                      </div>
                    </td>
                    <td className="px-5 py-3">
      ...
      >
            <div className="space-y-5">
              ...
              <div>
                ...
                <select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-[#2563EB]/20"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" placeholder={`Ej: Notebook HP EliteBook`}
                    className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none focus:ring-2 focus:ring-[#2563EB]/20"
                ...
                <div>
                  ...
                  <input type="text" placeholder={tabActivo === "Telefono" ? "IMEI del teléfono" : "Número de serie"}
                    className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none font-mono focus:ring-2 focus:ring-[#2563EB]/20"
                ...
                <div>
                  ...
                  <select className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none focus:ring-2 focus:ring-[#2563EB]/20"
